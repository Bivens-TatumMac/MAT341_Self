...
% give x-data points
xVec = [linspace(-1,0,floor(N/6)) linspace(0,1,ceil(2/3*N)) linspace(1,2,ceil(N/6))]; 
% gives N-equally spaced points between [-1,2]
...
yVec = r4.*exp( -(xVec-mu.*r3).^2 ./ (2*sigma^2.*r2) ) + r1; 
% computes randomly assorted y-data for each corresponding x-Pt
...
fs = 18; % FontSize
ms = 20; % MarkerSize
plot(xVec,yVec,'k.','MarkerSize',ms); 
hold on;
leg=legend('data');
xlabel('x');
ylabel('y');
